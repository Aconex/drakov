    ...
    var getResponseHandler = function (specPairs) {
        return function (req, res) {
            var buildResponseBody = function(specBody){
                if (req.serverName === 'restify') {
                    // restify needs a plain object to be sent, not a buffer
                    if (typeof specBody === 'string') {
                        try {
                            specBody = JSON.parse(specBody);
                        }
                        catch(e) {
                            // content type is not JSON, so keep it as a string
                        }
                    }
                    return specBody;
                }
