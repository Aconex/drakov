        ...
        return function (req, res, next) {
            if (req.method === 'GET' || content.isContentTypeEqual(req, originalReq)) {
                if (content.isBodyEqual(req, originalReq, content.getContentTypeFrom(req.headers))){
                    ...
                    console.log('[DRAKOV]'.red, method.green, url.yellow, (originalReq && originalReq.description ? originalReq.description : action.name).blue);

                    originalResp.headers.forEach(function(header){
                        res.set(header.name, header.value);
                    });
//                    console.log('response: ' +originalResp.name, originalResp.body);
                ...
                console.log('Skip. Different content-types for '.yellow, method.green,  url.blue);
                next();
            }
