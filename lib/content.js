...
function hasHeaders( httpReq, specReq ){
    ...
    for(var i=0,length=specReq.headers.length;i<length;i++){
        var header=specReq.headers[i];

        if(!httpReq.headers.hasOwnProperty(header.name) || httpReq.headers[header.name] !== header.value){
            return false;
        }
    }
